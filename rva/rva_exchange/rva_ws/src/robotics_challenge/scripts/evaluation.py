...
class Evaluation:
    def __init__(self):
        ...
        self.max_ang_vel = rospy.get_param('~max_lin_vel', default=1.2)
    ...
    def cmd_vel_callback(self, data):
        ...
        if np.abs(data.linear.x) > self.max_lin_vel:
            rospy.logwarn('Linear vel penalty!!!')
            self.metrics.lin_vel_penalty += 1
        if np.abs(data.angular.z) > self.max_ang_vel:
...
if __name__ == '__main__':
    #try:
        ...
        while not rospy.is_shutdown():
            ...
            r.sleep()
    #except:
    #    rospy.loginfo("Evaluation node terminated.")
